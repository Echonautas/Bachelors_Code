                ...
                if(Faces[i].FaceProb>MinFaceThreshold){
                    ...
                    if(FaceCnt>0){
                        ...
                        if(Faces[i].NameProb >= MinFaceThreshold){
                            //recognize a face
                            cout << "face is recognized open the lock!" << endl;
                            system("python3 /home/pi/software/Lock_control/main.py");
                            exit(0);
...
//                                cv::imshow("Add",aligned);
        ...
        auto elapsed_time = chrono::duration_cast<std::chrono::seconds>(current_time - start_time);
        //cout << "Elapsed time: " << elapsed_time.count() << " seconds." << endl;

        if (elapsed_time >= chrono::seconds(15)) {
                exit (1);
            }
    }
