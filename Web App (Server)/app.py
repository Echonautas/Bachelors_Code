...
def delete_directory():
    ...
    if request.method == 'POST':
        ...
        #Check if user has access to any lock
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("SELECT lock_id FROM users WHERE name = ?", (directory_name,))
        checking = c.fetchall()
        if checking == None:
            return render_template('delete.html', directories=existing_directories, message='Directory not found.')
        conn.close()

        #delete on selected lock
    ...
    return render_template('delete.html', directories=existing_directories, checking = checking)
        ...
        try:
            ...
            if host_data is None:
                ...
                if check_data is None:
                    ...
                    flash(f'User aleady has access!', 'error')
                    return redirect(url_for('run_command'))
...
@app.route('/delete_checking', methods=['GET', 'POST'])
def delete_checking():
    data = request.get_json()
    
    
    
    lock_ip = data['lock_ip']
    ...
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.execute("SELECT hostname, username, password FROM raspi WHERE hostname=?", (lock_ip,))
    host_data = c.fetchone()
    try:
            # Connect to the selected host from the dropdown menu
            c.execute("SELECT hostname, username, password FROM raspi WHERE hostname=?", (lock_ip,))
            host_data = c.fetchone()
            if host_data is None:
                print('Invalid host selected!', 'error')
                return redirect(url_for('run_command'))
            else:
                hostname, username, password = host_data
                ssh.connect(hostname, username=username, password=password)
            ...
            # for file_name in os.listdir(source_path):
            stdin, stdout, stderr = ssh.exec_command("cd /home/pi/software/Face-Recognition/img/ && rm -f {}*".format(directory_name))
            output = stdout.read().decode('utf-8')
            error = stderr.read().decode('utf-8')

            if error:
                print('Error delteting from initialization:' , error, ', error')

            else:
                print("Deleted from initialization: " , output, ", success")
                
                stdin, stdout, stderr = ssh.exec_command("rm -r /home/pi/software/Face-Recognition/Users/" + str(directory_name))   
                output = stdout.read().decode('utf-8')
                error = stderr.read().decode('utf-8')

                if error:
                    print('Error deleting backup' , error, ', error')

                else:
                    print("Deleted backup successfully: " , output, ", success")   

                try:
                    c.execute("DELETE FROM users WHERE lock_id=? AND name=?", (lock_ip, directory_name))
                    conn.commit()
